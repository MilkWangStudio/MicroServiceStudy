...
我们假设有一个叫王牛奶的同学，他在互联网初期用Java创建了一个叫“HiiroFans”的视频网站。  
...
#### 前后端分离
因为只有王牛奶一个人开发，所以前后端是模板引擎的合作方式。无论是页面、接口修改，都在一个项目里处理掉了，一个访问首页的接口大概是这样：
...
王牛奶开发的很开心，有什么功能先后端在一个项目里，一梭子就搞完了。后来王牛奶招了一个前端来帮他分担工作。这位前端小伙伴只会写JS，不会Java。没法改Java项目里的模板页面。    

经过沟通后他们决定前端小伙先写静态页面，写完之后把代码发给王牛奶，然后王牛奶把页面代码改成JSP模板引擎的语法。  

![模板引擎工作方式](./static/Working-Template.png)  

两个人开发迭代果然快很多，网站用户变多了，又开始扩充团队。这时候王牛奶开始忙不过来了，前端想改个颜色都得让王牛奶重新发布后端项目。王牛奶总结了下这种视图层和接口层混在一起的方案有这么几个问题：
...
这时候开始尝试把前端项目单独部署，王牛奶在线上给前端团队开了一台服务器，页面请求会被路由到这台服务器上，浏览器打开后再通过HTTP请求查询后台服务。整个请求的链路大概是这样的：  
...
因此王牛奶开始了第二轮的架构优化，从功能模块来拆分，比如音视频模块、用户模块、评论模块等，每个模块独立部署，内部完成这个模块的业务逻辑，相互之间没有调用。  
...
首先是项目过多，现有的后端团队没办法维护的过来了，另一个问题就是线上需要更多地机器部署，cicd流水线也变多了，运维团队忙不过来了。好在这时候有投资机构看中了这个项目，投了不少钱，于是王牛奶开始大力招人，HiiroFans发展迅猛。  

### 阶段四：引入服务聚合层（BFF）
随着融资进行，王牛奶开始把目光瞄准手机端市场，PC虽然体验很好，但是现在手机使用时长已经远远大于PC，所以开始做APP、H5的产品。  
